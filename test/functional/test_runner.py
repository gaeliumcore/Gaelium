...
`test/functional/test_framework/test_framework.py:GaeliumTestFramework.main`.
...
def main():
    ...
    tmpdir = "%s/gaelium_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    # Check that the build was configured with wallet, utils, and gaeliumd
    enable_wallet = config["components"].getboolean("ENABLE_WALLET")
    enable_cli = config["components"].getboolean("ENABLE_UTILS")
    enable_gaeliumd = config["components"].getboolean("ENABLE_GAELIUMD")
    if not (enable_wallet and enable_cli and enable_gaeliumd):
        print("No functional tests to run. Wallet, utils, and gaeliumd must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, use_term_control, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, last_loop=False):
    # Warn if gaeliumd is already running (unix only)
    if args is None:
        args = []
    try:
        if subprocess.check_output(["pidof", "gaeliumd"]) is not None:
            print("%sWARNING!%s There is already a gaeliumd process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    #Set env vars
    if "GAELIUMD" not in os.environ:
        os.environ["GAELIUMD"] = build_dir + '/src/gaeliumd' + exeext
        os.environ["GAELIUMCLI"] = build_dir + '/src/gaelium-cli' + exeext
...
class RPCCoverage:
    ...
    commands per `gaelium-cli help` (`rpc_interface.txt`).
