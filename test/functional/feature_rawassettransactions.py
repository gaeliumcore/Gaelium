...
class RawAssetTransactionsTest(GaeliumTestFramework):
    ...
    def issue_tampering_test(self):
        ...
        tx.deserialize(f)
        gaelo = '72766e6f'  # gaelo
        # remove the owner output from vout
        bad_vout = list(filter(lambda out_script: gaelo not in bytes_to_hex_str(out_script.scriptPubKey), tx.vout))
        tx.vout = bad_vout
        ...
        owner_vout = list(filter(lambda out_script: gaelo in bytes_to_hex_str(out_script.scriptPubKey), tx.vout))[0]
        ...
        for n in range(0, len(tx.vout)):
            out = tx.vout[n]
            if gaelo in bytes_to_hex_str(out.scriptPubKey):
                owner_out = out
                owner_script_hex = bytes_to_hex_str(owner_out.scriptPubKey)
                asset_script_hex = owner_script_hex[owner_script_hex.index(gaelo) + len(gaelo):-len(op_drop)]
                f = BytesIO(hex_str_to_bytes(asset_script_hex))
                owner = CScriptOwner()
                owner.deserialize(f)
                owner.name = b"NOT_MY_ASSET!"
                tampered_owner = bytes_to_hex_str(owner.serialize())
                tampered_script = owner_script_hex[:owner_script_hex.index(gaelo)] + gaelo + tampered_owner + op_drop
                tx.vout[n].scriptPubKey = hex_str_to_bytes(tampered_script)
        tx_bad_issue = bytes_to_hex_str(tx.serialize())
        tx_bad_issue_signed = n0.signrawtransaction(tx_bad_issue)['hex']
        ...
        tx.deserialize(f)
        gaelo = '72766e6f'  # gaelo
        GAELO = '4741454c4f'  # GAELO
        # change the owner output script type to be invalid
        for n in range(0, len(tx.vout)):
            out = tx.vout[n]
            if gaelo in bytes_to_hex_str(out.scriptPubKey):
                owner_script_hex = bytes_to_hex_str(out.scriptPubKey)
                tampered_script = owner_script_hex.replace(gaelo, GAELO)
                tx.vout[n].scriptPubKey = hex_str_to_bytes(tampered_script)
        tx_bad_issue = bytes_to_hex_str(tx.serialize())
        tx_bad_issue_signed = n0.signrawtransaction(tx_bad_issue)['hex']
        assert_raises_rpc_error(-26, "bad-txns-op-gael-asset-not-in-right-script-location",
                                n0.sendrawtransaction, tx_bad_issue_signed)
    ...
    def atomic_swaps_test(self):
        ...
        # n1 buys 400 ANDUIN from n2 for 4000 GAEL
        price = 4000
        amount = 400
        fee = 0.01
        ...
        # n2 buys JAINA! from n1 for 20000 GAEL
        price = 20000
        amount = 1
    ...
    def fundrawtransaction_transfer_outs(self):
        ...
        for _ in range(0, 5):
            n0.sendtoaddress(n2_address, gael_amount / 5)
        n0.generate(1)
        self.sync_all()
