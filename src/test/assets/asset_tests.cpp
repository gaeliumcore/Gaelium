...
BOOST_FIXTURE_TEST_SUITE(asset_tests, BasicTestingSetup)
    ...
    BOOST_AUTO_TEST_CASE(name_validation_tests)
    {
        ...
        //- Versions of GAELIUM NOT allowed
        BOOST_CHECK(!IsAssetNameValid("GAEL", type));
        BOOST_CHECK(!IsAssetNameValid("GAELIUM", type));
        BOOST_CHECK(!IsAssetNameValid("GAELIUM", type));

        //- Versions of GAELIUM ALLOWED
        BOOST_CHECK(IsAssetNameValid("GAELIUM.COIN", type));
        BOOST_CHECK(IsAssetNameValid("GAELIUM_COIN", type));
        BOOST_CHECK(IsAssetNameValid("GAELSPYDER", type));
        BOOST_CHECK(IsAssetNameValid("SPYDERGAEL", type));
        BOOST_CHECK(IsAssetNameValid("GAELIUMSPYDER", type));
        BOOST_CHECK(IsAssetNameValid("SPYDEGAELIUM", type));
        BOOST_CHECK(IsAssetNameValid("BLACK_GAELIUMS", type));
        BOOST_CHECK(IsAssetNameValid("SEGAELOT", type));
    ...
    BOOST_AUTO_TEST_CASE(transfer_asset_coin_test)
    {
        ...
        CAssetTransfer asset("GAELIUM", 1000);
    ...
    BOOST_AUTO_TEST_CASE(new_asset_coin_test)
    {
        ...
        // Create the asset scriptPubKey
        CNewAsset asset("GAELIUM", 1000, 8, 1, 0, "");
        CScript scriptPubKey = GetScriptForDestination(DecodeDestination(GetParams().GlobalBurnAddress()));
        asset.ConstructTransaction(scriptPubKey);
