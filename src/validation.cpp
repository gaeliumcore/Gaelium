...
#if defined(NDEBUG)
# error "Gaelium cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Gaelium Signed Message:\n";
                              ...
                              bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool test_accept)
{
...

    {
        ...
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s GAEL additional fees, %d delta bytes\n",
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    ...
    if (nHeight == 1)           return (10000000LL * COIN) + (1000 * COIN); // Premine 1% + base reward
    if (nHeight >= 3300001)     return 5 * COIN;       // perpetual tail emission
    if (nHeight >= 3150001)     return 20 * COIN;      // ~7 months   (3,000,000 GAEL)
    if (nHeight >= 2625001)     return 30 * COIN;      // ~24 months  (15,750,000 GAEL)
    if (nHeight >= 2100001)     return 50 * COIN;      // ~24 months  (26,250,000 GAEL)
    if (nHeight >= 1575001)     return 100 * COIN;     // ~24 months  (52,500,000 GAEL)
...
void UpdateCoins(const CTransaction& tx, CCoinsViewCache& inputs, CTxUndo &txundo, int nHeight, uint256 blockHash, CAssetsCache* assetCache, std::pair<std::string, CBlockAssetUndo>* undoAssetData)
{
    ...
    if (!tx.IsCoinBase()) {
        ...
        for (const CTxIn &txin : tx.vin) {
            ...
            bool is_spent = inputs.SpendCoin(txin.prevout, &txundo.vprevout.back(), assetCache); /** GAEL START */ /* Pass assetCache into function */ /** GAEL END */
            assert(is_spent);
        }
    }
    // add outputs
    AddCoins(inputs, tx, nHeight, blockHash, false, assetCache, undoAssetData); /** GAEL START */ /* Pass assetCache into function */ /** GAEL END */
}
...
//                                      << ", " << i << ", " << hash.GetHex() << ", " << k << ", " << "true" << ", " << assetAmount << std::endl;
        ...
        for (size_t o = 0; o < tx.vout.size(); o++) {
            if (!tx.vout[o].scriptPubKey.IsUnspendable()) {
                ...
                Coin coin;
                bool is_spent = view.SpendCoin(out, &coin, &tempCache); /** GAEL START */ /* Pass assetsCache into the SpendCoin function */ /** GAEL END */
        ...
        if (i > 0) { // not coinbases
            ...
            for (unsigned int j = tx.vin.size(); j-- > 0;) {
                ...
                int res = ApplyTxInUndo(std::move(undo), view, out, assetsCache); /** GAEL START */ /* Pass assetsCache into ApplyTxInUndo function */ /** GAEL END */
...
//                                      << ", " << i << ", " << txhash.GetHex() << ", " << k << ", " << "true" << ", " << assetAmount << std::endl;
    ...
    // Consensus enforcement: block 1 coinbase must pay the premine address
    if (pindex->nHeight == 1 && chainparams.NetworkIDString() == "main") {
        CKeyID premineKeyID(uint160(ParseHex("b07f0ede2d438c7f0c5d46e4c4d4d22e14ff5737")));
        CScript premineScript = GetScriptForDestination(premineKeyID);
        if (block.vtx[0]->vout[0].scriptPubKey != premineScript)
            return state.DoS(100,
                             error("ConnectBlock(): block 1 coinbase must pay premine address GZw9Z3ESiRqycCqwur1pohBinS5DRfgCrH"),
                             REJECT_INVALID, "bad-cb-premine");
    }
...
bool static FlushStateToDisk(const CChainParams& chainparams, CValidationState &state, FlushStateMode mode, int nManualPruneHeight) {
    ...
    try {
    {
        ...
        if (fDoFullFlush) {
            ...
            if (!CheckDiskSpace((48 * 2 * 2 * pcoinsTip->GetCacheSize()) + assetDirtyCacheSize * 2)) /** GAEL START */ /** GAEL END */
                return state.Error("out of disk space");
...
bool static ConnectTip(CValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexNew, const std::shared_ptr<const CBlock>& pblock, ConnectTrace& connectTrace, DisconnectedBlockTransactions &disconnectpool)
{
...

    {
        ...
        LogPrint(BCLog::BENCH, "  - Flush GAEL: %.2fms [%.2fs (%.2fms/blk)]\n", (nTime4 - nTime3) * MILLI, nTimeFlush * MICRO, nTimeFlush * MILLI / nBlocksTotal);
...
bool TestBlockValidity(CValidationState& state, const CChainParams& chainparams, const CBlock& block, CBlockIndex* pindexPrev, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    if (!ConnectBlock(block, state, &indexDummy, viewNew, chainparams, &assetCache, true)) /** GAEL START */ /*Add asset to function */ /** GAEL END*/
...
static bool RollforwardBlock(const CBlockIndex* pindex, CCoinsViewCache& inputs, const CChainParams& params, CAssetsCache* assetsCache = nullptr)
{
    ...
    // Skip genesis block (coinbase unspendable)
