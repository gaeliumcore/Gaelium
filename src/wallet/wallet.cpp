...
{
    // Skip genesis block coinbase - never count it
    if (IsCoinBase() && GetHash() == uint256S("9280011d752efed0c25a1d8a3fbd5d9ba50b953cac65f994b9d95437c9be6cfe"))
        return 0;
...
CAmount CWallet::GetBalance() const
{
    CAmount nTotal = 0;
    {
        ...
        for (std::map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            if (pcoin->IsCoinBase() && pcoin->GetHash() == uint256S("9280011d752efed0c25a1d8a3fbd5d9ba50b953cac65f994b9d95437c9be6cfe"))
                continue;
            if (pcoin->IsTrusted())
...
void CWallet::AvailableCoinsAll(std::vector<COutput>& vCoins, std::map<std::string, std::vector<COutput> >& mapAssetCoins, bool fGetGAEL, bool fGetAssets, bool fOnlySafe, const CCoinControl *coinControl, const CAmount& nMinimumAmount, const CAmount& nMaximumAmount, const CAmount& nMinimumSumAmount, const uint64_t& nMaximumCount, const int& nMinDepth, const int& nMaxDepth) const {
    vCoins.clear();

    {
        ...
        for (std::map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it) {
            ...
            // Skip genesis block coinbase - never spendable
            if (pcoin->IsCoinBase() && pcoin->GetHash() == uint256S("9280011d752efed0c25a1d8a3fbd5d9ba50b953cac65f994b9d95437c9be6cfe"))
                continue;
            ...
            for (unsigned int i = 0; i < pcoin->tx->vout.size(); i++) {
                ...
                if (fGetGAEL) { // Looking for GAEL Tx OutPoints Only
                    if (fGAELLimitHit) // We hit our limit
                        continue;

                    // We only want GAEL OutPoints. Don't include Asset OutPoints
                    if (isAssetScript)
                        continue;
                                   ...
                                   const AssetType& assetType, bool sign)
{
    ...
    unsigned int nBytes;
    {
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            // change transaction isn't always pay-to-gaelium-address
            ...
            while (true)
            {
                ...
                for (const auto& recipient : vecSend)
                {
                    ...
                    /** GAEL START */
                    // Check to see if you need to make an asset data outpoint OP_GAEL_ASSET data
                    ...
                    if (IsDust(txout, ::dustRelayFee) && !IsScriptTransferAsset(recipient.scriptPubKey)) /** GAEL START */ /** GAEL END */
                    {
