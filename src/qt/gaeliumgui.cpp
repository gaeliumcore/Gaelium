...
#include "config/gaelium-config.h"
#endif

#include "gaeliumgui.h"

#include "gaeliumunits.h"
...
const std::string GaeliumGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString GaeliumGUI::DEFAULT_WALLET = "~Default";
...
std::array<CurrencyUnitDetails, 5> CurrencyUnits::CurrencyOptions = { {
    { "BTC",    "GAELBTC"  , 1,          8},
    { "mBTC",   "GAELBTC"  , 1000,       5},
    { "µBTC",   "GAELBTC"  , 1000000,    2},
    { "Satoshi","GAELBTC"  , 100000000,  0},
    { "USDT",   "GAELUSDT" , 1,          5}
} };

static bool ThreadSafeMessageBox(GaeliumGUI *gui, const std::string& message, const std::string& caption, unsigned int style);

GaeliumGUI::GaeliumGUI(const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
...

{
    ...
    if(darkModeEnabled)
        statusBar()->setStyleSheet(".QStatusBar{background-color: #111827; border-top: 1px solid rgba(13, 181, 132, 0.1);}");
...
void GaeliumGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Gaelium address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and gaelium: URIs)"));
...
// DISABLED:     tabGroup->addAction(createAssetAction);
    ...
    transferAssetAction->setStatusTip(tr("Transfer assets to GAEL addresses"));
...
// DISABLED:     tabGroup->addAction(transferAssetAction);
...
// DISABLED:     tabGroup->addAction(manageAssetAction);
...
// DISABLED:     tabGroup->addAction(restrictedAssetAction);
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Gaelium addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Gaelium addresses"));
    ...
    openAction->setStatusTip(tr("Open a gaelium: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Gaelium command-line options").arg(tr(PACKAGE_NAME)));
...
void GaeliumGUI::createToolBars()
{
    if(walletFrame)
    {
        QSettings settings;
        ...
        QString widgetStyleSheet = ".QWidget {background-color: #111827; border-right: 1px solid rgba(13, 181, 132, 0.12);}";
        ...
        if(IconsOnly) {
            labelToolbar->setPixmap(QPixmap::fromImage(QImage(":/icons/gaeltext")));
        }
        else {
            labelToolbar->setPixmap(QPixmap::fromImage(QImage(":/icons/gaeliumtext")));
        }
...
// DISABLED:         m_toolbar->addAction(createAssetAction);
// DISABLED:         m_toolbar->addAction(transferAssetAction);
// DISABLED:         m_toolbar->addAction(manageAssetAction);
//        m_toolbar->addAction(messagingAction);
//        m_toolbar->addAction(votingAction);
// DISABLED:         m_toolbar->addAction(restrictedAssetAction);
        ...
        QString tbStyleSheet = ".QToolBar {background-color: transparent; border: none; }  "
                               ".QToolButton {background-color: transparent; border: none; width: 249px; color: %1; border-radius: 10px; padding: 12px 14px; margin: 1px 8px; font-size: 13px;} "
                               ".QToolButton:checked {background-color: rgba(13, 181, 132, 0.12); color: %2; border: none; border-left: 3px solid #0db584; border-radius: 10px; font: %4} "
                               ".QToolButton:hover {background-color: rgba(13, 181, 132, 0.06); border: none; border-radius: 10px; color: %3;} "
        ...
        QVBoxLayout* gaeliumLabelLayout = new QVBoxLayout(toolbarWidget);
        gaeliumLabelLayout->addWidget(labelToolbar);
        gaeliumLabelLayout->addWidget(m_toolbar);
        gaeliumLabelLayout->setDirection(QBoxLayout::TopToBottom);
        gaeliumLabelLayout->addStretch(1);

        QString mainWalletWidgetStyle = ".QWidget{background-color: #0a0f1a;}";
        ...
        QString widgetBackgroundSytleSheet = ".QWidget{background-color: #111827; border-bottom: 1px solid rgba(13, 181, 132, 0.1);}";
        ...
        labelCurrentMarket->setText(tr("Gaelium Market Price"));
        ...
        labelVersionUpdate->setText("<a href=\"https://github.com/gaeliumcore/Gaelium/releases\">New Wallet Version Available</a>");
...
// DISABLED:         mainFrameLayout->addWidget(headerWidget);
#ifdef ENABLE_WALLET
        ...
        // Get the latest Gaelium release and let the user know if they are using the latest version
        // Network request code for the header widget
                       ...
                       if (key == "tag_name") {
                           ...
                           if (fNewSoftwareFound) {
                               ...
                               if (GetRandInt(2) == 1) {
                                   ...
                                   if (fRet) {
                                       QString link = "https://github.com/gaeliumcore/Gaelium/releases";
...
void GaeliumGUI::updateIconsOnlyToolbar(bool IconsOnly)
{
    if(IconsOnly) {
        labelToolbar->setPixmap(QPixmap::fromImage(QImage(":/icons/gaeltext")));
        m_toolbar->setMaximumWidth(65);
...
void GaeliumGUI::setClientModel(ClientModel *_clientModel)
{
...
#ifdef ENABLE_WALLET
bool GaeliumGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool GaeliumGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void GaeliumGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void GaeliumGUI::setWalletActionsEnabled(bool enabled)
{
...
void GaeliumGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void GaeliumGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void GaeliumGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void GaeliumGUI::optionsClicked()
{
...
void GaeliumGUI::showDebugWindow()
{
...
void GaeliumGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void GaeliumGUI::showWalletRepair()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_REPAIR);
    showDebugWindow();
}

void GaeliumGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void GaeliumGUI::openClicked()
{
...
void GaeliumGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void GaeliumGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void GaeliumGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void GaeliumGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void GaeliumGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void GaeliumGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}

/** GAEL START */
void GaeliumGUI::gotoAssetsPage()
{
    transferAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoAssetsPage();
};

void GaeliumGUI::gotoCreateAssetsPage()
{
    createAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoCreateAssetsPage();
};

void GaeliumGUI::gotoManageAssetsPage()
{
    manageAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoManageAssetsPage();
};

void GaeliumGUI::gotoRestrictedAssetsPage()
{
...
void GaeliumGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Gaelium network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void GaeliumGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void GaeliumGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void GaeliumGUI::updateHeadersSyncProgressLabel()
{
...
void GaeliumGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void GaeliumGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("Gaelium"); // default title
...
void GaeliumGUI::changeEvent(QEvent *e)
{
...
void GaeliumGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void GaeliumGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void GaeliumGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& assetName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date);
    if (assetName == "GAEL")
        msg += tr("Amount: %1\n").arg(GaeliumUnits::formatWithUnit(unit, amount, true));
    else
        msg += tr("Amount: %1\n").arg(GaeliumUnits::formatWithCustomName(assetName, amount, MAX_ASSET_UNITS, true));
...
void GaeliumGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void GaeliumGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool GaeliumGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool GaeliumGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void GaeliumGUI::setHDStatus(int hdEnabled)
{
    QString icon = "";
...
void GaeliumGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void GaeliumGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void GaeliumGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void GaeliumGUI::detectShutdown()
{
...
void GaeliumGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void GaeliumGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void GaeliumGUI::showModalOverlay()
{
...
static bool ThreadSafeMnemonic(GaeliumGUI *gui, unsigned int style)
{
    bool modal = (style & CClientUIInterface::MODAL);
    // The SECURE flag has no effect in the Qt GUI.
    // bool secure = (style & CClientUIInterface::SECURE);
...
void GaeliumGUI::subscribeToCoreSignals()
{
...
void GaeliumGUI::unsubscribeFromCoreSignals()
{
...
void GaeliumGUI::toggleNetworkActive()
{
    if (clientModel) {
...
void GaeliumGUI::handleRestart(QStringList args)
{
    if (!ShutdownRequested())
    ...
    menu(0)
{
    ...
    QList<GaeliumUnits::Unit> units = GaeliumUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const GaeliumUnits::Unit unit : units)
    {
    #ifndef QTversionPreFiveEleven
        max_width = qMax(max_width, fm.horizontalAdvance(GaeliumUnits::name(unit)));
    #else
        max_width = qMax(max_width, fm.width(GaeliumUnits::name(unit)));
    #endif
    }
...
{
    menu = new QMenu(this);
    for (GaeliumUnits::Unit u : GaeliumUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(GaeliumUnits::name(u)), this);
...
{
    setText(GaeliumUnits::name(newUnits));
}
...
void GaeliumGUI::currencySelectionChanged(int unitIndex)
{
    if(clientModel && clientModel->getOptionsModel())
    {
...
void GaeliumGUI::onCurrencyChange(int newIndex)
{
    qDebug() << "GaeliumGUI::onPriceUnitChange: " + QString::number(newIndex);
...
void GaeliumGUI::getLatestVersion()
{
    versionRequest->setUrl(QUrl("https://api.github.com/repos/gaeliumcore/Gaelium/releases"));
