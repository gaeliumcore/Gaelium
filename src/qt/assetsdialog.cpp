...
void AssetsDialog::setupAssetControlFrame(const PlatformStyle *platformStyle)
{
    ...
    ui->frameAssetControl->setStyleSheet(".QFrame {background-color: #111827; border: 1px solid rgba(13, 181, 132, 0.1); border-radius: 12px; padding: 16px; border: none;}");
...
void AssetsDialog::assetControlChangeEdited(const QString& text)
{
    if (model && model->getAddressTableModel())
    {
        ...
        {
            ui->labelAssetControlChangeLabel->setText(tr("Warning: Invalid Gaelium address"));
        }
        else // Valid address
        {
