...
//        ui->optInRBF->setCheckState(model->getDefaultWalletRbf() ? Qt::Checked : Qt::Unchecked);
        ...
        ui->reissueCostLabel->setText(tr("Cost") + ": " + GaeliumUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), GetBurnAmount(AssetType::REISSUE)));
...
void ReissueAssetDialog::setupAssetDataView(const PlatformStyle *platformStyle)
{
    ...
    ui->frame->setStyleSheet(".QFrame {background-color: #111827; border: 1px solid rgba(13, 181, 132, 0.1); border-radius: 12px; padding: 16px; border: none;}");
    ...
    ui->frame_3->setStyleSheet(".QFrame {background-color: #111827; border: 1px solid rgba(13, 181, 132, 0.1); border-radius: 12px; padding: 16px; border: none;}");
    ui->frame_3->setGraphicsEffect(GUIUtil::getShadowEffect());

    ui->frame_2->setStyleSheet(".QFrame {background-color: #111827; border: 1px solid rgba(13, 181, 132, 0.1); border-radius: 12px; padding: 16px; border: none;}");
...
void ReissueAssetDialog::CheckFormState()
{
    ...
    if (!ui->addressText->text().isEmpty()) {
        if (!IsValidDestination(dest)) {
            showMessage(tr("Invalid Gaelium Destination Address"));
            return;
        }
    }
...
void ReissueAssetDialog::onReissueAssetClicked()
{
    ...
    QString amount = "<b>" + QString::fromStdString(ValueFromAmountString(GetReissueAssetBurnAmount(), 8)) + " GAEL";
    amount.append("</b>");
