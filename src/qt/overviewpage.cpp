...
public:
    explicit TxViewDelegate(const PlatformStyle *_platformStyle, QObject *parent=nullptr):
        QAbstractItemDelegate(parent), unit(GaeliumUnits::GAEL),
    ...
    assetdelegate(new AssetViewDelegate(platformStyle, this))
{
    ...
    ui->assetFrame->setStyleSheet(".QFrame {background-color: #111827; border: 1px solid rgba(13, 181, 132, 0.1); border-radius: 16px; padding: 16px;}");
    ui->frame->setStyleSheet(".QFrame {background-color: qlineargradient(x1:0, y1:0, x2:1, y2:1, stop:0 #0c2a3d, stop:0.4 #0a1628, stop:0.7 #112040, stop:1 #0d1f35); border: 1px solid rgba(13, 181, 132, 0.2); border-radius: 16px; padding: 20px;}");
    ui->frame_2->setStyleSheet(".QFrame {background-color: #111827; border: 1px solid rgba(13, 181, 132, 0.1); border-radius: 16px; padding: 16px;}");
    ...
    ui->gaelBalancesLabel->setStyleSheet("color: #e8ecf4; font-size: 18px;");
    ui->labelBalanceText->setStyleSheet("color: #8a94a8;");
    ui->labelPendingText->setStyleSheet("color: #8a94a8;");
    ui->labelImmatureText->setStyleSheet("color: #8a94a8;");
    ui->labelTotalText->setStyleSheet("color: #0db584; font-size: 14px;");
    ui->labelSpendable->setStyleSheet("color: #8a94a8;");
    ui->labelWatchonly->setStyleSheet("color: #8a94a8;");
    ui->recentTransactionsLabel->setStyleSheet("color: #e8ecf4; font-size: 18px;");

    /** Update the labels font */
    ui->gaelBalancesLabel->setFont(GUIUtil::getTopLabelFont());
    ...
    ui->assetSearch->setStyleSheet(".QLineEdit {border: 1px solid rgba(13, 181, 132, 0.2); border-radius: 8px; background-color: #0a0f1a; color: #e8ecf4; padding: 6px 12px;}");
...
void OverviewPage::setBalance(const CAmount& balance, const CAmount& unconfirmedBalance, const CAmount& immatureBalance, const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    ui->labelBalance->setText(GaeliumUnits::formatWithUnit(unit, balance, false, GaeliumUnits::separatorAlways));
    ui->labelUnconfirmed->setText(GaeliumUnits::formatWithUnit(unit, unconfirmedBalance, false, GaeliumUnits::separatorAlways));
    ui->labelImmature->setText(GaeliumUnits::formatWithUnit(unit, immatureBalance, false, GaeliumUnits::separatorAlways));
    ui->labelTotal->setText(GaeliumUnits::formatWithUnit(unit, balance + unconfirmedBalance + immatureBalance, false, GaeliumUnits::separatorAlways));
    ui->labelWatchAvailable->setText(GaeliumUnits::formatWithUnit(unit, watchOnlyBalance, false, GaeliumUnits::separatorAlways));
    ui->labelWatchPending->setText(GaeliumUnits::formatWithUnit(unit, watchUnconfBalance, false, GaeliumUnits::separatorAlways));
    ui->labelWatchImmature->setText(GaeliumUnits::formatWithUnit(unit, watchImmatureBalance, false, GaeliumUnits::separatorAlways));
    ui->labelWatchTotal->setText(GaeliumUnits::formatWithUnit(unit, watchOnlyBalance + watchUnconfBalance + watchImmatureBalance, false, GaeliumUnits::separatorAlways));
...
void OverviewPage::setWalletModel(WalletModel *model)
{
    ...
    // update the display unit, to not use the default ("GAEL")
    updateDisplayUnit();
}
...
void OverviewPage::showAssets()
{
    if (AreAssetsDeployed()) {
        ...
        // This keeps the GAEL balance grid from expanding and looking terrible when asset balance is hidden
