...
#include "gaeliumaddressvalidator.h"
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Gaelium address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(GetParams()))));
#endif
    widget->setValidator(new GaeliumAddressEntryValidator(parent));
    widget->setCheckValidator(new GaeliumAddressCheckValidator(parent));
}
...
bool parseGaeliumURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no gaelium: URI
    if(!uri.isValid() || uri.scheme() != QString("gaelium"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!GaeliumUnits::parse(GaeliumUnits::GAEL, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseGaeliumURI(QString uri, SendCoinsRecipient *out)
{
    // Convert gaelium:// to gaelium:
    //
    //    Cannot handle this later, because gaelium:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("gaelium://", Qt::CaseInsensitive))
    {
        uri.replace(0, 10, "gaelium:");
    }
    QUrl uriInstance(uri);
    return parseGaeliumURI(uriInstance, out);
}

QString formatGaeliumURI(const SendCoinsRecipient &info)
{
    QString ret = QString("gaelium:%1").arg(info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(GaeliumUnits::format(GaeliumUnits::GAEL, info.amount, false, GaeliumUnits::separatorNever));
        paramCount++;
    }
...
bool openGaeliumConf()
{
    boost::filesystem::path pathConfig = GetConfigFile(GAELIUM_CONF_FILENAME);
    ...
    /* Open gaelium.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Gaelium.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Gaelium (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Gaelium (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for Gaelium*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = ChainNameFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "gaelium.desktop";
    return GetAutostartDir() / strprintf("gaelium-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = ChainNameFromCommandLine();
        // Write a gaelium.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Gaelium\n";
        else
            optionFile << strprintf("Name=Gaelium (%s)\n", chain);
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    ...
    // loop through the list of startup items and try to find the gaelium app
...
bool GetStartOnSystemStartup()
{
    CFURLRef gaeliumAppUrl = CFBundleCopyBundleURL(CFBundleGetMainBundle());
    if (gaeliumAppUrl == nullptr) {
        return false;
    }
    
    LSSharedFileListRef loginItems = LSSharedFileListCreate(nullptr, kLSSharedFileListSessionLoginItems, nullptr);
    LSSharedFileListItemRef foundItem = findStartupItemInList(loginItems, gaeliumAppUrl);

    CFRelease(gaeliumAppUrl);
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add gaelium app to startup item list
        LSSharedFileListInsertItemURL(loginItems, kLSSharedFileListItemBeforeFirst, nullptr, nullptr, gaeliumAppUrl, nullptr, nullptr);
    }
