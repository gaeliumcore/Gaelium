...
bool GenerateDistributionList(const CRewardSnapshot& p_rewardSnapshot, std::vector<OwnerAndAmount>& vecDistributionList)
{
    ...
    CAmount srcUnitDivisor = COIN;  //  Default to divisor for GAEL
    ...
    if (p_rewardSnapshot.strDistributionAsset != "GAEL") {
        ...
        LogPrint(BCLog::REWARDS, "%s: Distribution is GAEL with divisor %d\n", __func__, srcUnitDivisor);
    }
        ...
        std::string& change_address, uint256& retTxid)
{
    ...
    //  Handle payouts using GAEL differently from those using an asset
    if (p_rewardSnapshot.strDistributionAsset == "GAEL") {
        // Check amount
        ...
        for (int i = start; i < (int)p_pendingPayments.size() && i < stop; i++) {
            ...
            // Parse Gaelium address (already validated during ownership snapshot creation)
