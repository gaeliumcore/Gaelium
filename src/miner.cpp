...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fMineWitnessTx)
{
    ...
    // Premine: force block 1 coinbase to fixed address on mainnet
    if (chainparams.NetworkIDString() == "main" && nHeight == 1) {
    // P2PKH premine to GZw9Z3ESiRqycCqwur1pohBinS5DRfgCrH
    // hash160(pubkey) = b07f0ede2d438c7f0c5d46e4c4d4d22e14ff5737
    CKeyID keyID(uint160(ParseHex("b07f0ede2d438c7f0c5d46e4c4d4d22e14ff5737")));
    CScript premineScript = GetScriptForDestination(keyID);
    if (!premineScript.empty()) {
        coinbaseTx.vout[0].scriptPubKey = premineScript;
    }
}
...
void static GaeliumMiner(const CChainParams& chainparams)
{
    LogPrintf("GaeliumMiner -- started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("gaelium-miner");
    ...
    if (!EnsureWalletIsAvailable(pWallet, false)) {
        LogPrintf("GaeliumMiner -- Wallet not available\n");
    }
#endif
    ...
    try {
        ...
        while (true) {
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("GaeliumMiner -- Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;
            IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

            LogPrintf("GaeliumMiner -- Running miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("GaeliumMiner -- terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("GaeliumMiner -- runtime error: %s\n", e.what());
        return;
    }
}

int GenerateGaeliums(bool fGenerate, int nThreads, const CChainParams& chainparams)
{
    ...
    for (int i = 0; i < nThreads; i++){
        minerThreads->create_thread(boost::bind(&GaeliumMiner, boost::cref(chainparams)));
    }
