...
The Gaelium Doxygen documentation can be generated by running `make docs` from the top-level 
...
Run with the -testnet option to run with "play gaeliums" on the test network, if you
...
Gaelium Core is a multithreaded application, and deadlocks or other multithreading bugs
...
- GaeliumMiner : Generates gaeliums (if wallet is enabled).
...
However, in open source software such as Gaelium Core, where everyone uses
...
pay attention to for reviewers of Gaelium Core code.

General Gaelium Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Gaelium Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against Gaelium so that they can be integrated
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:gaelium/gaelium.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `gaelium-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `gaelium-cli` and the GUI debug console use this table to determine how to
