...
The Gaelium Core daemon can be configured to act as a trusted "border
router", implementing the gaelium wire protocol and relay, making
...
The ZeroMQ feature in Gaelium Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building gaeliumd:
...
For instance:
    ...
    $ gaeliumd -zmqpubhashtx=tcp://127.0.0.1:28766 \
               -zmqpubrawtx=ipc:///tmp/gaeliumd.tx.raw
...
These options can also be provided in gaelium.conf.
...
From the perspective of gaeliumd, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into gaeliumd directly. Furthermore, no information is
...
using. Gaeliumd appends an up-counting sequence number to each
