...
The RPM spec file provided here is for Gaelium-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://gaelium.io/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Gaelium git repository](https://github.com/gaeliumcore/Gaelium) but are not part of
...
Currently only `Source10` is used, for the example `gaelium.conf` file.
...
in the gaelium git repository. At present only an SVG version of the Gaelium
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the gaelium
...
needed when 0.12.1 is released, a proper fix is already in the Gaelium git
master branch.
...
idea to build Gaelium Core against a static version of that library compiled
...
The `gaelium.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `gaelium-server` package. At this point, most RPM based
...
The `gaelium-server` package should have SELinux support. How to properly do
...
Gaelium-Core against OpenSSL or use a patched version of Boost in the build
system.
...
4. I renamed the `gaelium` package that contains the Qt GUI to `gaelium-core` as
that appears to be how the general population refers to it, in contrast to
`gaelium-xt` or `gaelium-classic`. I wanted to make sure the general population
